// Replit Auth integration for TFive enterprise authentication
...
export function getSession() {
  ...
  const sessionStore = new pgStore({
    ...
    ttl: sessionTtl,
    tableName: "auth_sessions", // Using auth_sessions to avoid conflict with user sessions
  });
  return session({
...
async function upsertUser(claims: any) {
  // Map Replit Auth claims to our user schema
  await storage.upsertUser({
    id: claims["sub"],
    email: claims["email"],
    username: claims["email"]?.split("@")[0] || `user_${claims["sub"]}`,
    displayName: [claims["first_name"], claims["last_name"]].filter(Boolean).join(" ") || null,
